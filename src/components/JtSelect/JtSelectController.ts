...
import { AsyncComponent, Component as VueComponent, VNode } from 'vue';
import {
  Component,
  Prop,
  Vue,
...
export class JtSelectController extends Vue {
  ...
  */
  ...
  afterDeselect(option: SelectOption): void {
    if (this.disabled || this.readonly) return;
    this.$emit('after-deselect', { option });
  }

  /** Handle updates after the selection changes */
  afterSelect(option: SelectOption, wasCreated?: boolean): void {
    if (this.disabled || this.readonly) return;
  ...
  beforeDeselect(option: SelectOption): void {
    if (this.disabled || this.readonly) return;
  ...
  /** Handle updates before the selection changes */
  beforeSelect(option: SelectOption, willCreate?: boolean): void {
    if (this.disabled || this.readonly) return;
   ...
   */
  inputKeyDown(ev: KeyboardEvent): void {
   ...
   */
  selectOption(option?: SelectOption, create?: boolean): void {
   ...
   */
  updateSelection(options: SelectOption | SelectOption[] | null): void {
    if (this.disabled || this.readonly) return;
